...
export function creditLines(credits) {
  ...
  if (credits.monthlyCredits > 0 || credits.purchasedCredits > 0 || credits.freeCredits > 0) {
    ...
    lines.push(`balance: ${money(credits.monthlyCredits)} monthly, ${money(credits.purchasedCredits)} purchased, ${money(credits.freeCredits)} free`)
  }
  for (const [label, window] of [['5h window', credits.fiveHour], ['weekly window', credits.weekly]]) {
    ...
    // These are dollars, not request counts: the caps are round money limits,
    // and a window's `used` tracks what the requests inside it cost.
    lines.push(`${label}: $${window.used.toFixed(2)} used of $${window.cap}${window.exceeded ? ' (exceeded)' : ''}${reset}`)
  }
  return lines
}
