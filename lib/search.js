 ...
 *     directly, and disabling it puts back whatever it displaced. The toggle is
 *     the user saying "use this account for search", so it takes the seat the
 *     way any explicit choice does; a deployment that wants the other provider
 *     leaves the toggle off, which is exactly the untouched default.
 */
 ...
 * @returns `(enable) => 'taken' | 'released'`. Enabling always takes the seat,
 *   remembering what was there; disabling restores it. The remembered value is
 *   only restored while this plugin still owns the seat, so a selection changed
 *   in the meantime is never clobbered on the way out.
 */
