...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      usageMonthlyDetail: (left, used) => `剩 ${left} / 本期已用 ${used}（额度）`,
      usageWindowDetail: (used, cap) => `已用 ${formatAmount(used)} / 上限 ${formatAmount(cap)}（额度）`,
    ...
    const en = {
      ...
      usageMonthlyDetail: (left, used) => `${left} left / ${used} used this period (credits)`,
      usageWindowDetail: (used, cap) => `${formatAmount(used)} used / ${formatAmount(cap)} max (credits)`,
