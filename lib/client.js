...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      usageWeekly: '每周窗口',
      usageCreditsMonthly: '月度额度',
      usageMonthlyDetail: (left, used) => `剩 ${left} · 本期已用 ${used}`,
      usageWindowDetail: (used, cap) => `已用 ${formatAmount(used)} / 上限 ${formatAmount(cap)}`,
      usageCreditsPurchased: '购买额度',
      usageCreditsFree: '赠送额度',
    ...
    const en = {
      ...
      usageCreditsMonthly: 'Monthly',
      usageMonthlyDetail: (left, used) => `${left} left · ${used} used this period`,
      usageWindowDetail: (used, cap) => `${formatAmount(used)} used / ${formatAmount(cap)} max`,
    ...
    const CSS = [
      ...
      '.ccg-meterDetail{grid-area:2/2;text-align:right;font-size:11px;color:var(--dsw-alias-label-tertiary,inherit);font-variant-numeric:tabular-nums}',
      '.ccg-bar{grid-area:2/1;height:6px;border-radius:999px;background:var(--dsw-alias-bg-layer-4,rgba(127,127,127,.2));overflow:hidden}',
      ...
      '.ccg-percent{grid-area:1/2;text-align:right;font-size:12px;font-weight:600;font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-primary,inherit)}',
    ...
    /**
     * A credit balance. The service reports these at full precision
     * (69.985238253), which is noise in a metric tile — two places is what the
    ...
    /**
     * A quantity at two places without trailing zeros: `1.51`, `14`, `0.5`.
     * Used for the figures beside a quota, where the service's full precision
     * (1.51498977) is noise, and a fixed `.00` on an integer cap would read as
     * spurious accuracy.
     */
     ...
     * checked rather than taken on trust. Neither source states what the two
     * figures are denominated in, so the share is the only form in which they
    ...
    function UsagePanel(props) {
      ...
      const windowMeter = (window, label) => {
        ...
        return h(MeterRow, {
          ...
          detail: t.usageWindowDetail(window.used, window.cap),
          ...
          : h('div', { className: 'ccg-cardBody' },
            ...
            monthly === undefined
              ...
              : h(MeterRow, {
                ...
                detail: t.usageMonthlyDetail(formatCredits(monthly.left), formatCredits(monthly.used)),
                t,
              }),
            h('div', { className: 'ccg-metrics' },
              report.credits === undefined || !(report.credits.purchasedCredits > 0)
                ? null
                : metric(t.usageCreditsPurchased, formatCredits(report.credits.purchasedCredits)),
              report.credits === undefined || !(report.credits.freeCredits > 0)
                ? null
                : metric(t.usageCreditsFree, formatCredits(report.credits.freeCredits)),
    ...
    function CommandCodeCard(props) {
      ...
      return h('div', { className: 'ccg-root' },
        ...
        h('section', { className: 'ccg-section' },
          h(Button, { variant: 'ghost', size: 'sm', onClick: () => setProvidersOpen(!providersOpen) },
            `${providersOpen ? '▾' : '▸'} ${t.targets}`),
          providersOpen ? h('div', { className: 'ccg-card' }, providerDetail()) : null),
