...
额度以**百分比**呈现：窗口直接给出 `used` / `cap`，月度池则只给出「剩余」与「本期已用」，分母由两者相加得出——服务端从不直接给总量。百分比领先显示，原始数字以小字跟在旁边：两个来源都没说明这些数目的单位，百分比是唯一能把它们摆在一起比较的形式。取整按量级走（`0.02%` 不会被抹成 `0%`，`20.4%` 也不需要多余精度）。
